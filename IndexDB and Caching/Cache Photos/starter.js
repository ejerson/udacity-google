...
// a variable that holds the name of the image cache
var contentImgsCache = 'wittr-content-imgs';

// array of cache names we care about
var allCaches = [
  staticCacheName,
  contentImgsCache
];
      ...
      return Promise.all(
        cacheNames.filter(function(cacheName) {
          return cacheName.startsWith('wittr-') &&
                  // delete all the caches that is included inside the
                  // array of caches on line 7
...
self.addEventListener('fetch', function(event) {
  ...
  if (requestUrl.origin === location.origin) {
    ...
    // handles a url with a patch /photos/
